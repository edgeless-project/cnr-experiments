...
import numpy as np
from utils import show_or_save, map_physical_to_workflow_id, relabel
...
wf_duration = (
    df.groupby(["experiment", "size"]).max()["timestamp"]
    - df.groupby(["experiment", "size"]).min()["timestamp"]
).max()
print(df.groupby(["experiment", "size"]).max()["timestamp"])
print(df.groupby(["experiment", "size"]).min()["timestamp"])
...
wf_duration = 60.0
new_df["norm_tpt"] = new_df["wid"] / (new_df["num_workflows"] * wf_duration)
